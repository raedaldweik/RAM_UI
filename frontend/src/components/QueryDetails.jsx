...
function Section({ title, count, children, defaultOpen = false }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(0,114,206,0.14)' }}>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-2 px-4 py-2.5 text-left transition-all hover:bg-[rgba(0,114,206,0.04)]"
        style={{ background: 'rgba(0,114,206,0.05)' }}>
        ...
        {count != null && (
          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-bold"
            style={{ background: 'rgba(0,114,206,0.10)', color: 'var(--gold)' }}>{count}</span>
        )}
      </button>
...
const Pill = ({ children }) => (
  <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold"
    style={{ background: 'rgba(0,114,206,0.08)', color: 'var(--gold-lo)', border: '1px solid rgba(0,114,206,0.15)' }}>
    {children}
  </span>
);
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-6" onClick={onClose}
      ...
      <div className="glass-card w-full max-w-[760px] max-h-[85vh] flex flex-col animate-slide-up"
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(0,114,206,0.10)]">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[rgba(0,114,206,0.08)]" style={{ color: 'var(--text-dim)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          ...
          {/* Question */}
          <div className="rounded-xl px-4 py-3" style={{ background: 'rgba(0,114,206,0.05)', border: '1px solid rgba(0,114,206,0.14)' }}>
          ...
          <Section title="Tool calls" count={toolCalls.length} defaultOpen={toolCalls.length > 0}>
            ...
            {toolCalls.map((c, i) => (
              <div key={c.id || i} className="rounded-lg px-3 py-2.5 space-y-2" style={{ border: '1px solid rgba(0,114,206,0.10)' }}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0"
                    style={{ background: 'rgba(0,114,206,0.12)', color: 'var(--gold)' }}>{i + 1}</span>
          ...
          <Section title="Retrieved context" count={context.length}>
            ...
            {context.map((doc, i) => {
              ...
              return (
                <div key={i} className="rounded-lg px-3 py-2.5" style={{ border: '1px solid rgba(0,114,206,0.10)' }}>
