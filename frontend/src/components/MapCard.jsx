 ...
 * themed to match the SAS RAM UI (pearl/glass chrome, SAS blue overlays).
 *
 * Data source: the `tomtom-render-map` MCP tool. The spec carries Dubai/UAE
...
const SAS_BLUE = '#0072CE';      // primary — routes, pins, traffic toggle
const SAS_BLUE_HI = '#2B95E8';   // lighter blue — category dots
const TEAL = '#0e7490';          // secondary accent — area / isochrone overlays
const INK = '#0A2540';           // SAS deep navy — text
        ...
        try {
          for (const l of map.getStyle().layers || []) {
            if (l.type === 'fill-extrusion' && /building/i.test(l.id)) {
              ...
              map.setPaintProperty(l.id, 'fill-extrusion-color', '#e2e8f0');
